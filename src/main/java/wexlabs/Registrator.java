...
public class Registrator {
    ...
    public static void onGrowthLogicKitRegistry(final RegistryEvent<GrowthLogicKit> event) {
        if (event.isEntryOfType(GrowthLogicKit.class)){
           event.getRegistry().registerAll(JUNIPER_GROWTH_LOGIC, LONG_BRANCH_CANOPY_GROWTH_LOGIC, UP_AND_OUT_GROWTH_LOGIC, BAOBAB_GROWTH_LOGIC, CYPRESS_GROWTH_LOGIC, REDWOOD_GROWTH_LOGIC, SMALL_REDWOOD_GROWTH_LOGIC, WILLOW_GROWTH_LOGIC, CANOPY_GROWTH_LOGIC);
        }
    ...
    public static void onCellKitRegistry(final RegistryEvent<CellKit> event) {
        if (event.isEntryOfType(CellKit.class)){
            event.getRegistry().registerAll(CellKits.POPLAR, CellKits.DOME, CellKits.JOSHUA, CellKits.WILLOW);
        }
    }
}
